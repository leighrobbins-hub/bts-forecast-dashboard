/**
 * Orbital Academy — Middle School Math (space theme).
 * Same XP economy as Gene Code: 4 units × (5 missions × 100 + 4 quiz × 25) = 2400 max.
 */
(function (global) {
  var STORAGE_KEY = "orbitalAcademy_v1";
  var LESSON_IDS = ["launch", "transfer", "sensors", "geometry"];
  var XP_MISSION = 100;
  var XP_QUIZ = 25;
  ...
  function displayName() {
    var n = state.name && String(state.name).trim() ? String(state.name).trim() : "Cadet";
  ...
  function beep(ok) {
    ...
    try {
      ...
      osc.frequency.value = ok ? 720 : 200;
      osc.type = "sine";
      g.gain.value = 0.07;
      osc.start();
  ...
  function refreshLessonHUD(lessonId) {
    ...
    if (cn) {
      cn.textContent = "Call sign: " + displayName();
  ...
  global.OrbitalGame = {
    ...
    setName: function (n) {
      state.name = String(n || "Cadet").slice(0, 24);
      save();
    },
    setMute: function (m) {
      state.mute = !!m;
